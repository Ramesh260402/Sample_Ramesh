...
public class GreedyAlgorithm {
    public static void main(String[] args) {
        Item[] items = {
            ...
            new Item("1 light and stretchy long-sleeve shirt (Gap Fit)", 23, 147),
        ...
        while (i < items.length && totalWeight + items[i].weight <= maxWeight) {
        ...
        System.out.println("");
        System.out.println("Total worth of selected items: " + totalWorth);
        ...
        for (Item item : selectedItems) {
            System.out.printf("%-60s%-8d%d%n", item.name, item.worth, item.weight);
        }
    }
}
...
class Item implements Comparable<Item> {
    ...
    public int compareTo(Item other) {
        return Integer.compare(other.worth, this.worth);
    }
}
